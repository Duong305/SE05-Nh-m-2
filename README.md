...
Thực hiện việc vẽ hoặc xoay hình bằng các thuật toán:          
  * File allViews.py được lấy dữ liệu đầu vào từ file allView_data.xml là để vẽ ra các tấm gỗ hay dc gọi là các thành phần của tủ các tấm đó từ cái tủ dc tách ra thành nhiều tấm     cách nhau 1 khoảng nhất định và chú thích tên của     từng tấm vào đúng vị trí của tấm đó
  ...
  * File XoayCacTam.py được lấy dữ liệu đầu vào từ file allView_data.xml là để xoay các tấm đó quanh các trục x,y,z sử dụng thuật toán xoay hình 3d liên quan đến lượng giác
    ...
    + Tiếp theo chúng mình cũng sẽ tạo ra 1 list chứa toạ độ điểm chiều dài, chiều rộng, chiều cao của, các góc xoay quanh trục của mỗi tấm, rồi bằng thuật toán xoay hình 3d           chúng mình sẽ thay đổi các giá trị x, các giá trị y, các giá trị z của tọa độ điểm mỗi tấm. Sau đó chúng mình lưu hết các giá trị x, y, z đó vào 1 list rỗng khác.
  ...
  * File bezierAlgo.py được lấy dữ liệu đầu vào từ file bezierAlgo_data.svg sử dụng thuật toán hay phương trình đường cong bezier để vẽ ra các đường cong từ việc nối rất nhiều       đoạn thẳng cực nhỏ giữa các đoạn thẳng đó có độ chênh lệch nghiêng cực nhỏ tức phương trình trên biến các điểm điều khiển đường cong thành nhiều điểm cực sát nhau có quỹ         đạo trở thành đường cong rồi nối trúng lại với nhau bhằng phương thức add_line() của ezdxf là phương thức nối đoạn thẳng giữa 2 điểm
